...
const image = {
  uri: "https://media.istockphoto.com/id/479762254/photo/app-icons.jpg?b=1&s=170667a&w=0&k=20&c=DX1DtiuAL-_viBPRiFl-3DSG0tYA62o6HVe-6t04B4s=",
};
...
const RequestAccept = (props) => {
  ...
  const Read = () => {
    ...
    const myDoc = doc(db, "Newusers", "Sumon Ahmed");
  ...
  const [todos, setTodos] = useState(["alif", "ola"]);
  const [input, setInput] = useState([]);

  useEffect(() => {
    // db.collection('Users').onSnapshot(snapshot=>{
    //     setTodos(snapshot.docs.map(doc=>doc.data().location));
    // })
  }, []);
        ...
        <Card style={styles.view}>
          {userDoc != null && (
            <View>
              ...
              <Text
                ...
                onPress={() => navigation.navigate("Addcart", item)}
              >
                Contact Number : {userDoc.contactnumber}
              </Text>
              <Text
                style={styles.text}
                onPress={() => navigation.navigate("Addcart", item)}
          ...
          <View>
            {/* <Button
                ...
                /> */}
            <TouchableOpacity style={[{ marginLeft: 10, height: 60 }]}>
              <Text style={{ marginLeft: 100 }}>
                <Ionicons
                  ...
                  onPress={() => {
                    ...
                    alert("CONGRATULATIONS!!! Have a Safe Ride..");
                  }}
                  color="black"
                />
