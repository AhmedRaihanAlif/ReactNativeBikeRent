...
const Cart = ({ navigation }) => {
  ...
  useEffect(
    ()=>{
      ...
      onSnapshot(ref,(catagories)=>
      setData(catagories.docs.map((item)=>({
        data:item.data()
      })))
      )
    }
  ...
  const [bike, setBike] = useState([
    ...
    // {name: "Pulsar ", bikername: "Md Amir", bikecost: "250", bikemilage: "40",contactnumber: "01523901070",address: "Mirpur,Dhaka",image:image3, key: "2" },
  ...
  return (
    ...
    <View>
                  ...
                  <Text
                    ...
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Bike Cost: {item.bikecost} tk per hour
                  </Text>
                  <Text
                    style={styles.text}
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Bike Milage: {item.bikemilage} kmh
                  </Text>
                </View>
                 }
              ...
              {userDoc != null && (
                <View>
                  <Text
                    ...
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Biker Name: {userDoc.name};
                  </Text>
                  <Text
                    style={styles.text}
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Bike Name : {userDoc.bikename}
                  </Text>
                  <Text
                    style={styles.text}
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Bike Cost: {userDoc.bikecost} tk per hour
                  </Text>
                  <Text
                    style={styles.text}
                    onPress={() => navigation.navigate("Addcart", item)}
                  >
                    Bike Milage: {userDoc.bikemilage} kmh
                  </Text>
                </View>
              )}
